...
<view class="trtc-room-container">
  <block v-if="template === '1v1'">
    <block data-type="template" data-is="1v1" data-attr="pusher, streamList, debug">
  <view class="template-1v1">
    ...
    <view class="bottom-btns">
      ...
      <view class="btn-normal" @tap="_toggleIMPanel">
        <image class="btn-image" src="/static/components/trtc-room/template/1v1/static/im.png"></image>
      </view>
    </view>
  </view>
</block>
  ...
  <block v-if="template === 'grid'">
    <block data-type="template" data-is="grid" data-attr="pusher, streamList, visibleStreamList, debug, panelName, gridPagePlaceholderStreamList, gridPageCount, gridCurrentPage, gridPlayerPerPage">
  ...
  <view class="template-custom">
    <view class="players-container">
      <view v-for="(item, streamID) in streamList" :key="streamID" v-if="item.src && (item.hasVideo || item.hasAudio)" :class="'view-container player-container ' + (item.isVisible?'':'none')" :style="'left:' + item.xAxis + 'top:' + item.yAxis + 'width:' + item.width + 'height:' + item.height + 'z-index:' + item.zIndex + ''">
    ...
    <view :class="'view-container pusher-container ' + (pusher.isVisible?'':'none')" :style="'left:' + pusher.xAxis + 'top:' + pusher.yAxis + 'width:' + pusher.width + 'height:' + pusher.height + 'zindex:' + pusher.zindex + ''">
      <live-pusher class="pusher" :url="pusher.url" :mode="pusher.mode" :autopush="pusher.autopush" :enable-camera="pusher.enableCamera" :enable-mic="pusher.enableMic" :muted="!pusher.enableMic" :enable-agc="pusher.enableAgc" :enable-ans="pusher.enableAns" :enable-ear-monitor="pusher.enableEarMonitor" :auto-focus="pusher.enableAutoFocus" :zoom="pusher.enableZoom" :min-bitrate="pusher.minBitrate" :max-bitrate="pusher.maxBitrate" :video-width="pusher.videoWidth" :video-height="pusher.videoHeight" :beauty="pusher.beautyLevel" :whiteness="pusher.whitenessLevel" :orientation="pusher.videoOrientation" :aspect="pusher.videoAspect" :device-position="pusher.frontCamera" :remote-mirror="pusher.enableRemoteMirror" :local-mirror="pusher.localMirror" :background-mute="pusher.enableBackgroundMute" :audio-quality="pusher.audioQuality" :audio-volume-type="pusher.audioVolumeType" :audio-reverb-type="pusher.audioReverbType" :waiting-image="pusher.waitingImage" :debug="debug" @statechange="_pusherStateChangeHandler" @netstatus="_pusherNetStatusHandler" @error="_pusherErrorHandler" @bgmstart="_pusherBGMStartHandler" @bgmprogress="_pusherBGMProgressHandler" @bgmcomplete="_pusherBGMCompleteHandler"></live-pusher>
    </view>
  </view>
</block>
  ...
  <view class="im-panel" v-if="enableIM && showIMPanel">
    <view class="message-panel-body">
      <scroll-view scroll-y="true" class="message-scroll-container" :scroll-into-view="'message' + (messageList.length-1)" scroll-with-animation="true">
        <view class="message-list">
          <view class="message-item" v-for="(item, index) in messageList" :key="index" :id="'message' + index">
    ...
    <view class="message-panel-bottom">
      <view class="message-input-container">
        <input class="message-input" type="text" :value="messageContent" @input="inputIMMessageFun" @confirm="sendIMMessageFun" confirm-type="send" placeholder="请输入消息" maxlength="200" placeholder-style="color:#ffffffopacity: 0.55"></input>
      </view>
  ...
  <view :class="'debug-info ' + (debugMode && debugPanel?'':'none')">
    ...
    <view>debug:<button :class="(debug?'':'false') + ' debug-btn'" @tap="debugToggleVideoDebugFun" hover-class="button-hover">{{debug}}</button></view>
    <view>userID: {{pusher.userID}}</view>
    <view>roomID: {{pusher.roomID}}</view>
    <view>camera:<button :class="(pusher.enableCamera?'':'false') + ' debug-btn'" @tap="toggleVideoFun" hover-class="button-hover">{{pusher.enableCamera}}</button></view>
    <view>mic:<button :class="(pusher.enableMic?'':'false') + ' debug-btn'" @tap="toggleAudioFun" hover-class="button-hover">{{pusher.enableMic}}</button></view>
    <view>switch camera:<button class="debug-btn" @tap="switchCamera" hover-class="button-hover">{{cameraPosition||pusher.frontCamera}}</button></view>
    <view>Room:<button class="debug-btn" @tap="debugEnterRoomFun" hover-class="button-hover">Enter</button>
    ...
    <view>IM:<button class="debug-btn" @tap="debugSendRandomMessageFun" hover-class="button-hover">send</button></view>
    ...
    <view v-for="(item, streamID) in streamList" :key="streamID">{{item.userID}}|{{item.streamType}}|
      SubV:<button :class="(!item.muteVideo?'':'false') + ' debug-btn'" @tap="debugToggleRemoteVideoFun" hover-class="button-hover" :data-user-i-d="item.userID" :data-stream-type="item.streamType">{{!item.muteVideo}}</button>|
      SubA:<button :class="(!item.muteAudio?'':'false') + ' debug-btn'" @tap="debugToggleRemoteAudioFun" hover-class="button-hover" :data-user-i-d="item.userID" :data-stream-type="item.streamType">{{!item.muteAudio}}</button></view>
  </view>
</view>
</view>
</template>
...
export default {
  ...
  onPageShow: function () {
    ...
    if (this.status.isPending) {
      // 经历了 5000 挂起事件
      this.status.isPending = false; // 修复iOS 最小化触发5000事件后，音频推流失败的问题 20200409注释修复逻辑
    ...
    if (this.status.isPush) {// 小程序hide - show 有一定概率本地黑屏或静止，远端正常，或者远端和本地同时黑屏或静止，需要手动启动预览，非必现
  ...
  methods: {
    ...
    propertyObserverFun(data) {
      ...
      if (data.name === 'config') {
        ...
        console.log(TAG_NAME, '_propertyObserver config:', config); // 由于 querystring 只支持 String 类型，做一个类型防御
    ...
    getPushUrlFun(rtcConfig) {
      ...
      if (ENV.IS_TRTC) {
        ...
        return new Promise((resolve, reject) => {
          ...
          setTimeout(() => {
            const pushUrl = 'room://cloud.tencent.com/rtc?sdkappid=' + rtcConfig.sdkAppID + '&roomid=' + rtcConfig.roomID + '&userid=' + rtcConfig.userID + '&usersig=' + rtcConfig.userSig + '&appscene=' + rtcConfig.scene + '&encsmall=' + rtcConfig.encsmall + '&cloudenv=' + rtcConfig.cloudenv;
      ...
      return new Promise((resolve, reject) => {
        wx.request({
          ...
          success: res => {
            ...
            let pushUrl = 'room://cloud.tencent.com?sdkappid=' + sdkAppID + '&roomid=' + roomID + '&userid=' + userID + '&roomsig=' + encodeURIComponent(roomSig); // TODO 需要重新整理的逻辑 TRTC尚未支持 20200213
    ...
    pusherStateChangeHandlerFun(event) {
      ...
      switch (code) {
        ...
        case 5000:
          ...
          this.status.isPending = true; // if (this.status.isPush) {
          //   // this.exitRoom()
    ...
    filterVisibleStreamFun(streamList, skipPagination) {
      ...
      if (this.template === 'grid' && !skipPagination) {
        this.filterGridPageVisibleStreamFun(list);
    ...
    filterGridPageVisibleStreamFun(list) {
      ...
      if (this.gridPlayerPerPage > 3) {
        if (this.gridCurrentPage === 1) {
          ...
          // 2 7
          // 6 11
          interval = [this.gridCurrentPage * this.gridPlayerPerPage - (this.gridPlayerPerPage + 2), this.gridCurrentPage * this.gridPlayerPerPage - 1];
        }
      } else {
        // 每页显示3个，每页都有pusher
        interval = [this.gridCurrentPage * this.gridPlayerPerPage - (this.gridPlayerPerPage + 1), this.gridCurrentPage * this.gridPlayerPerPage];
      }
    ...
    onIMMessageReceivedFun(event) {
      ...
      for (let i = 0; i < messageData.length; i++) {
        const message = messageData[i]; // console.log(TAG_NAME, 'IM.MESSAGE_RECEIVED', message, this.data.config, TIM.TYPES.MSG_TEXT)
    ...
    onIMNotReadyFun(event) {
      ...
      this._emitter.emit(EVENT.IM_SDK_NOT_READY, event); // 收到 SDK 进入 not ready 状态通知，此时 SDK 无法正常工作
    ...
    gridBindEventFun() {
      ...
      this.on(EVENT.REMOTE_AUDIO_VOLUME_UPDATE, event => {
        ...
        const volume = data.detail.volume; // console.log(TAG_NAME, '远端音量变更', userID, streamType, volume, event)
